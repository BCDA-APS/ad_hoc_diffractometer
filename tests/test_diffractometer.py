...
@pytest.mark.parametrize("axis, angle_deg, expected, context", [
    pytest.param(XHAT,   0.0, np.eye(3), does_not_raise(), id="zero-angle-x-is-identity"),
    pytest.param(YHAT,   0.0, np.eye(3), does_not_raise(), id="zero-angle-y-is-identity"),
    pytest.param(ZHAT,   0.0, np.eye(3), does_not_raise(), id="zero-angle-z-is-identity"),
    ...
    pytest.param(XHAT, -45.0, Rx(-45),   does_not_raise(), id="negative-angle-about-x"),
    pytest.param(ZHAT, -90.0, Rz(-90),   does_not_raise(), id="negative-angle-about-z"),
    pytest.param(-ZHAT, 30.0, Rz(-30),   does_not_raise(), id="negated-axis-equals-negated-angle"),
    pytest.param(-XHAT, 45.0, Rx(-45),   does_not_raise(), id="negated-x-axis-equals-negated-angle"),
    pytest.param(2*XHAT, 90.0, Rx(90),   does_not_raise(), id="non-unit-axis-is-normalised"),
...
@pytest.mark.parametrize("axis, angle_deg, context", [
    ...
    pytest.param(np.array([1, 1, 1]),   60.0,  does_not_raise(), id="diagonal-axis-60deg"),
])
...
@pytest.mark.parametrize("name, axis, parent, role, angle, context", [
    pytest.param('mu',    XHAT,  None,  'sample',   0.0,  does_not_raise(), id="sample-floor-stage"),
    pytest.param('eta',  -ZHAT, 'mu',   'sample',  45.0,  does_not_raise(), id="sample-stacked-stage"),
    pytest.param('delta', -ZHAT, 'nu',  'detector', 0.0,  does_not_raise(), id="detector-stacked-stage"),
    pytest.param('phi',  -ZHAT,  None,  'sample',   0.0,  does_not_raise(), id="sample-floor-stage-neg-z"),
])
...
@pytest.mark.parametrize("axis, angle_deg, expected_R, context", [
    pytest.param(XHAT,   0.0,  np.eye(3), does_not_raise(), id="zero-angle-is-identity"),
    pytest.param(XHAT,  90.0,  Rx(90),    does_not_raise(), id="90deg-about-x"),
...
@pytest.mark.parametrize("basis, context", [
    pytest.param(
    ...
    pytest.param(
        {'lateral': np.array([0.0, 0.0, 0.0]), 'longitudinal': YHAT, 'vertical': ZHAT},
    ...
    pytest.param(
        {'lateral': np.array([1.0, 0.0, 0.0, 0.0]), 'longitudinal': YHAT, 'vertical': ZHAT},
        pytest.raises(ValueError, match=re.escape("3-dimensional")),
...
@pytest.mark.parametrize("stages, context", [
    pytest.param(
    ...
    pytest.param(
        ...
        pytest.raises(ValueError, match=re.escape("which is not in the stage list")),
...
@pytest.mark.parametrize("stages, role, expected_order, context", [
    pytest.param(
...
@pytest.mark.parametrize("stage_name, angle, context", [
    ...
    pytest.param('bogus', 10.0,  pytest.raises(KeyError), id="unknown-stage-raises-KeyError"),
])
...
@pytest.mark.parametrize("angles, expected_Z, context", [
    pytest.param(
        {'mu': 0, 'eta': 0, 'chi': 0, 'phi': 0}, np.eye(3), does_not_raise(),
        id="all-zero-is-identity",
    ),
    pytest.param(
        {'mu': 90, 'eta': 0, 'chi': 0, 'phi': 0}, Rx(90), does_not_raise(),
        id="mu-90-only-rotates-about-x",
    ),
    pytest.param(
        {'mu': 0, 'eta': 30, 'chi': 0, 'phi': 0}, Rz(-30), does_not_raise(),
        id="eta-30-only-left-handed-z",
    ),
    pytest.param(
        {'mu': 0, 'eta': 0, 'chi': 45, 'phi': 0}, Ry(45), does_not_raise(),
        id="chi-45-only-rotates-about-y",
    ),
    pytest.param(
        {'mu': 0, 'eta': 0, 'chi': 0, 'phi': 60}, Rz(-60), does_not_raise(),
...
@pytest.mark.parametrize("angles, expected_D, context", [
    pytest.param(
        {'nu': 0, 'delta': 0}, np.eye(3), does_not_raise(),
        id="all-zero-is-identity",
    ),
    pytest.param(
        {'nu': 90, 'delta': 0}, Rx(90), does_not_raise(),
        id="nu-90-only-rotates-about-x",
    ),
    pytest.param(
        {'nu': 0, 'delta': 45}, Rz(-45), does_not_raise(),
...
@pytest.mark.parametrize("factory, expected_name, sample_names, detector_names, context", [
    pytest.param(
        geometry_psic, 'psic',
    ...
    pytest.param(
        geometry_fourc, 'fourc',
        ['omega', 'chi', 'phi'], ['two_theta'],
        does_not_raise(),
...
def test_geometry_factories(factory, expected_name, sample_names, detector_names, context):
    with context:
        g = factory()
...
@pytest.mark.parametrize("factory, stage_name, expected_parent, context", [
    pytest.param(geometry_psic,  'mu',        None,    does_not_raise(), id="psic-mu-on-floor"),
    pytest.param(geometry_psic,  'nu',        None,    does_not_raise(), id="psic-nu-on-floor"),
    pytest.param(geometry_psic,  'eta',       'mu',    does_not_raise(), id="psic-eta-on-mu"),
    pytest.param(geometry_psic,  'chi',       'eta',   does_not_raise(), id="psic-chi-on-eta"),
    pytest.param(geometry_psic,  'phi',       'chi',   does_not_raise(), id="psic-phi-on-chi"),
    pytest.param(geometry_psic,  'delta',     'nu',    does_not_raise(), id="psic-delta-on-nu"),
    pytest.param(geometry_fourc, 'omega',     None,    does_not_raise(), id="fourc-omega-on-floor"),
    pytest.param(geometry_fourc, 'two_theta', None,    does_not_raise(), id="fourc-two_theta-on-floor-decoupled"),
    pytest.param(geometry_fourc, 'chi',       'omega', does_not_raise(), id="fourc-chi-on-omega"),
    pytest.param(geometry_fourc, 'phi',       'chi',   does_not_raise(), id="fourc-phi-on-chi"),
    pytest.param(geometry_sixc,  'alpha',     None,    does_not_raise(), id="sixc-alpha-on-floor"),
    pytest.param(geometry_sixc,  'omega',     'alpha', does_not_raise(), id="sixc-omega-on-alpha"),
    pytest.param(geometry_sixc,  'delta',     'alpha', does_not_raise(), id="sixc-delta-on-alpha"),
    pytest.param(geometry_sixc,  'gamma',     'delta', does_not_raise(), id="sixc-gamma-on-delta"),
])
...
@pytest.mark.parametrize("factory, stage_name, expected_axis, context", [
    pytest.param(geometry_psic, 'mu',    +XHAT, does_not_raise(), id="psic-mu-vertical-right-handed"),
    pytest.param(geometry_psic, 'eta',   -ZHAT, does_not_raise(), id="psic-eta-lateral-left-handed"),
    pytest.param(geometry_psic, 'chi',   +YHAT, does_not_raise(), id="psic-chi-longitudinal-right-handed"),
    pytest.param(geometry_psic, 'phi',   -ZHAT, does_not_raise(), id="psic-phi-lateral-left-handed"),
    pytest.param(geometry_psic, 'nu',    +XHAT, does_not_raise(), id="psic-nu-vertical-right-handed"),
    pytest.param(geometry_psic, 'delta', -ZHAT, does_not_raise(), id="psic-delta-lateral-left-handed"),
])
...
@pytest.mark.parametrize("a, b, c, alpha, beta, gamma, context", [
    pytest.param(5.0, 5.0, 5.0,  90.0,  90.0,  90.0, does_not_raise(), id="cubic"),
    pytest.param(2.0, 3.0, 4.0,  90.0,  90.0,  90.0, does_not_raise(), id="orthorhombic"),
    pytest.param(3.0, 3.0, 5.0,  90.0,  90.0, 120.0, does_not_raise(), id="hexagonal"),
    pytest.param(4.0, 5.0, 6.0,  90.0, 100.0,  90.0, does_not_raise(), id="monoclinic"),
])
...
@pytest.mark.parametrize("a, b, c, alpha, beta, gamma, context", [
    pytest.param(2.0, 3.0, 4.0, 90.0, 90.0, 90.0, does_not_raise(), id="orthorhombic-aligned-with-axes"),
])
...
@pytest.mark.parametrize("a, b, c, alpha, beta, gamma, context", [
    pytest.param(3.0, 3.0, 5.0,  90.0,  90.0, 120.0, does_not_raise(), id="hexagonal-gamma-120"),
    pytest.param(4.0, 5.0, 6.0,  90.0, 100.0,  90.0, does_not_raise(), id="monoclinic-beta-100"),
])
...
@pytest.mark.parametrize("a, b, c, context", [
...
@pytest.mark.parametrize("a, b, c, alpha, beta, gamma, h, context", [
    pytest.param(5.0, 5.0, 5.0,  90,  90,  90, np.array([1, 0, 0]), does_not_raise(), id="cubic-h100"),
    pytest.param(5.0, 5.0, 5.0,  90,  90,  90, np.array([0, 1, 0]), does_not_raise(), id="cubic-h010"),
    pytest.param(5.0, 5.0, 5.0,  90,  90,  90, np.array([0, 0, 1]), does_not_raise(), id="cubic-h001"),
    pytest.param(5.0, 5.0, 5.0,  90,  90,  90, np.array([1, 2, 3]), does_not_raise(), id="cubic-h123"),
    pytest.param(3.0, 3.0, 5.0,  90,  90, 120, np.array([1, 0, 0]), does_not_raise(), id="hexagonal-h100"),
    pytest.param(3.0, 3.0, 5.0,  90,  90, 120, np.array([1, 1, 0]), does_not_raise(), id="hexagonal-h110"),
    pytest.param(4.0, 5.0, 6.0,  90, 100,  90, np.array([0, 0, 1]), does_not_raise(), id="monoclinic-h001"),
])
