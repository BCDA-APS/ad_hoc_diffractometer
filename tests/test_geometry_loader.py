...
# ---------------------------------------------------------------------------
# REMOVE-AT-V1.0: loader rewriting of legacy reference-constraint names
# (issue #299).
# ---------------------------------------------------------------------------

_LEGACY_REFERENCE_YAML = """\
ad_hoc_diffractometer_geometry:
    schema_revision: 1
name: legacy_alias
documentation: A geometry whose YAML still uses the legacy reference names.
basis: BL
stages:
    - {name: mu,    axis: -transverse, parent: null,  role: sample}
...
def test_loader_rewrites_legacy_reference_names_emits_single_warning():
    """Loading YAML with legacy ``alpha_i`` / ``beta_out`` names emits one
    DeprecationWarning per file and produces a geometry with canonical
    constraint and extras keys.

    Issue #299.  An out-of-tree YAML file written against the v0.11.x
    API still uses the deprecated reference-constraint names; the loader
    rewrites them in-place and surfaces a single deprecation warning
    naming the file rather than per-constraint warnings from
    :class:`ReferenceConstraint.__init__`.
    """
    import warnings

    with warnings.catch_warnings(record=True) as captured:
        warnings.simplefilter("always")
        g = load_geometry_file(_LEGACY_REFERENCE_YAML)

    # Exactly one DeprecationWarning fires, from the loader's rewriting
    # path; the per-constraint warning from ReferenceConstraint.__init__
    # is suppressed because the loader rewrites the names before
    # construction.
    dep_warnings = [w for w in captured if issubclass(w.category, DeprecationWarning)]
    rewriting_warnings = [w for w in dep_warnings if "YAML rewritten" in str(w.message)]
    assert len(rewriting_warnings) == 1, (
        f"expected exactly one loader rewriting DeprecationWarning; got "
        f"{len(rewriting_warnings)}: {[str(w.message) for w in rewriting_warnings]}"
    )
    msg = str(rewriting_warnings[0].message)
    assert "alpha_i" in msg and "beta_out" in msg
    assert "incidence" in msg and "emergence" in msg
    assert "issue #299" in msg

    # Per-constraint warnings from ReferenceConstraint.__init__ are
    # absent (they would fire only if a legacy name reached the
    # constructor, which the loader rewrite prevents).
    per_constraint_warnings = [
        w for w in dep_warnings if "ReferenceConstraint name" in str(w.message)
    ]
    assert per_constraint_warnings == [], (
        f"loader rewrite should suppress per-constraint warnings; got "
        f"{[str(w.message) for w in per_constraint_warnings]}"
    )

    # The loaded geometry has the canonical constraint name and the
    # canonical extras keys (no legacy spellings).
    cs = g.modes["legacy_incidence"]
    from ad_hoc_diffractometer.mode import ReferenceConstraint

    ref_constraints = [c for c in cs.constraints if isinstance(c, ReferenceConstraint)]
    assert len(ref_constraints) == 1
    assert ref_constraints[0].name == "incidence"
    assert "incidence" in cs.extras
    assert "emergence" in cs.extras
    assert "alpha_i" not in cs.extras
    assert "beta_out" not in cs.extras


def test_loader_drops_legacy_extras_key_when_canonical_already_present():
    """If a YAML extras dict declares both the legacy and the canonical
    key (a malformed mixed declaration that nothing in the in-tree
    code produces but an out-of-tree author might write), the loader
    drops the legacy entry rather than overwriting the canonical one.
    The canonical slot's value wins; the legacy slot's value is
    discarded.
    """
    mixed_yaml = _LEGACY_REFERENCE_YAML.replace(
        "alpha_i: null\n            beta_out: null",
        "alpha_i: null\n            incidence: null\n            beta_out: null",
    )
    import warnings
    ...
    with warnings.catch_warnings(record=True) as captured:
        warnings.simplefilter("always")
        g = load_geometry_file(mixed_yaml)
    # Loader still fires the rewriting warning (legacy names were present).
    dep_warnings = [
        w
        for w in captured
        if issubclass(w.category, DeprecationWarning)
        and "YAML rewritten" in str(w.message)
    ]
    assert len(dep_warnings) == 1
    # Canonical key survives; legacy key is gone.
    cs = g.modes["legacy_incidence"]
    assert "incidence" in cs.extras
    assert "alpha_i" not in cs.extras


def test_loader_skips_rewriting_when_yaml_is_already_canonical():
    """No DeprecationWarning fires for YAML that already uses the canonical
    names — the rewriting path activates only when legacy names are
    found.  This protects in-tree YAML files (migrated to canonical
    names) from triggering spurious deprecation noise at every
    ``make_geometry()`` call.
    """
    import warnings

    canonical_yaml = _LEGACY_REFERENCE_YAML.replace("alpha_i", "incidence").replace(
        "beta_out", "emergence"
    )
    with warnings.catch_warnings(record=True) as captured:
        warnings.simplefilter("always")
        load_geometry_file(canonical_yaml)

    dep_warnings = [
        w
        for w in captured
        if issubclass(w.category, DeprecationWarning) and "issue #299" in str(w.message)
    ]
    assert dep_warnings == [], (
        f"canonical YAML must not trigger any issue-#299 deprecation "
        f"warnings; got {[str(w.message) for w in dep_warnings]}"
    )
