...
Five-circle diffractometer: a standard {ref}`geometry-fourcv` (Eulerian four-circle) mounted on a vertical mu base stage. Sample and detector are coupled through mu.
...
This geometry is defined by the {ref}`geometry-fivec` factory
function — see the [source](https://github.com/BCDA-APS/ad_hoc_diffractometer/blob/main/src/ad_hoc_diffractometer/geometries/fivec.yml) for the complete stage
...
(N − 3 = 2 for N = 5 DOF).
With mu = 0, {ref}`geometry-fivec` reduces identically to {ref}`geometry-fourcv`.
